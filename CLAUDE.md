...
**No hecho en esta sesión (quedó fuera de alcance a propósito, es Sesión 13B):** Tarea 4b (alertas por contexto individual / patrón base por cliente en "Seguimiento - Análisis Lunes") y Tarea 4c (dashboard para clientes finales, nueva auth + página `/cliente/dashboard`) — ver BACKLOG PRODUCTOS y el brief completo de sesión 13 para el detalle de ambas, ninguna se ha empezado a implementar todavía.
...
- [ ] **Sesión 13B (pendiente, no empezada)**: Tarea 4b (alertas por contexto individual — ampliar histórico de "Seguimiento - Análisis Lunes" de últimas 4 semanas a 12 meses, calcular "patrón base" por cliente, prompt de Claude comparando el reporte contra ese patrón en vez de umbrales genéricos) y Tarea 4c (dashboard para clientes finales — nueva tabla de auth en Supabase, `POST /api/clientes/login`, página `/cliente/dashboard` con gráficas de progreso, permisos para que el cliente solo vea sus propios datos). Ver el brief completo de sesión 13 para el detalle de ambas — decisión consciente de dividir en dos sesiones por volumen de tokens, se retoma cuando el usuario lo pida explícitamente
