...
│ │ ├── cliente/
│ │ │ └── dashboard/page.tsx # Dashboard del cliente final (NEW, sesión 13B): nombre/objetivo/entrenador, gráfica de peso (Recharts, 3 meses), entrenamientos vs objetivo + últimas 4 semanas, energía 30 días, próximo check-in, banner de alerta del entrenador. Header propio (no reutiliza Header.tsx, que es de entrenador/admin). Gate: getUser() + GET /api/cliente/perfil, redirect a /login si no hay cliente asociado al email
│ │ └── api/
