...
- [ ] **Verificar visualmente en `/dashboard`** que el cliente de prueba "Test Checkin Sprint" (entrenador `espartakofake@gmail.com`) aparece con alerta ⚠️ — dejado deliberadamente en Airtable sin borrar para esta comprobación manual (ver ESTADO ACTUAL, Tarea 2). Se puede borrar después de verificarlo (Cliente `recxdLuxwENwQZ4wV` + sus 4 Reportes)
- [ ] **Limpieza de datos de prueba antiguos en Airtable** (no tocado en esta sesión, fuera de alcance): hay registros corruptos/vacíos preexistentes de sesiones anteriores — `Clientes` `recIO8ikpWQFlJ9hr` (sin Nombre/Email, con `#ERROR!` en `Last_modified`) y `Reportes` `recyqm5KowgHopbp5` (mismo patrón) — y varios clientes de prueba con emails `@example.com`/duplicados ("gato"/"gato", "Juan" Perdido, etc.) acumulados de distintas sesiones. No bloquean nada, pero convendría una limpieza puntual antes de cualquier demo real a un entrenador
