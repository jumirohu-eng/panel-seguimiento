...
function ClienteSignupForm() {
  ...
  useEffect(() => {
    ...
    async function validate() {
      try {
        const res = await fetch(`/api/signup/cliente/validate?token=${encodeURIComponent(token!)}`)
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    await fetch('/api/signup/cliente/complete', {
      method: 'POST',
    ...
    <main className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="w-full max-w-sm rounded-xl border border-border bg-card p-8 shadow-sm">
        ...
        <p className="mb-6 text-center text-sm text-muted">
          Tu entrenador te ha invitado a RetainCoach — crea tu contraseña para acceder
        </p>
        ...
        {success && (
          <p className="text-center text-sm text-success">
            ✅ Cuenta creada. Revisa tu email para confirmarla y luego inicia sesión. Redirigiendo…
          </p>
        )}
      </div>
    </main>
